...
3. Brief feedback analyzing:
   ...
   - List the attributes like Top: ..., Bottom: ..., Pants: ..., etc. (include accessories)
   - Include the colors of the clothes
4. Specific, actionable suggestions for improvement (if any)
5. Shein shopping links where users can buy similar items with the same attributes. Use real Shein search links in the format:
   https://www.shein.tw/pdsearch/<search term length color>
   List them as Top: <link>, Bottom: <link>, Pants: <link>, etc.
...
{{
    ...
    "score": <number 1-10>,
    "feedback": "<detailed explanation including attributes and colors>",
    "suggestions": ["<suggestion1>", "<suggestion2>", ...],
    "shein_links": {{
        "top": "<shein search link>",
        "bottom": "<shein search link>",
        "pants": "<shein search link>",
        "shoes": "<shein search link>",
        "accessories": "<shein search link>"
    }}
}}"""
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.7,
            max_tokens=1000  # Increased to accommodate Shein links
        )
        ...
        # Ensure shein_links is included (even if empty)
        if 'shein_links' not in result:
            result['shein_links'] = {}
...
def process_image(
    ...
    """
    Main processing function for clothing analysis.
    This function can be called from API endpoints or other scripts.
    
    Args:
        image_path: Path to input image
        save_dir: Directory containing model checkpoints
        backbone: Backbone architecture (resnet50 or resnet18)
        evaluate_style: Whether to run ChatGPT style evaluation
        openai_key: OpenAI API key (optional, can use env var)
        visualize: Whether to generate visualization images
        output_dir: Directory to save output files
        
    Returns:
        Dictionary containing all analysis results with structure:
        {
            "success": bool,
            "categories": [...],
            "attributes": {...},
            "region_colors": {...},
            "color_confidence": {...},
            "style_evaluation": {...} (optional),
            "visualizations": {...} (paths to generated images)
        }
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Transforms for the attribute model
    ...
    if os.path.isfile(ckpt_path):
        ...
        print(f"[Warning] Attribute checkpoint not found at {ckpt_path}")
    attr_model = attr_model.to(device)

    # Parse clothing regions
    vis_path = os.path.join(output_dir, "parsed_regions_visualization.png") if visualize else None
    ...
    decoded_attrs = infer_single(attr_model, image_path, device, transforms_img, parsing_mask=None)
    vertical_region_colors = {}
    
    # Adjust masks based on classifier results (e.g., remove legs for shorts)
    ...
    # Extract colors using classifier predictions as guide
    classifier_colors = {
    ...
    parsing_region_colors = {
        ...
        "skirt": {"hex": None, "name": None, "pattern": None},
    }
    ...
    if region_masks["shirt"].sum() > 0:
        pred_mask[region_masks["shirt"]] = 1
    if region_masks["pants"].sum() > 0:
        pred_mask[region_masks["pants"]] = 5
    if region_masks["shoes"].sum() > 0:
        pred_mask[region_masks["shoes"]] = 11
    
    categories = generate_clothing_categories(pred_mask, parsing_region_colors, decoded_attrs)

    # Combine everything into one JSON result
    result = {
        "success": True,
        "categories": categories,
        "attributes": decoded_attrs,
        "region_colors": matched_colors,
        "color_confidence": {
            "overall": float(overall_color_confidence),
            "per_region": {k: float(v) for k, v in color_confidences.items()}
        },
        "region_colors_vertical": vertical_region_colors,
        "visualizations": {
            "parsed_regions": vis_path
        }
    }
    
    # Style evaluation with ChatGPT (optional)
    if evaluate_style:
        ...
        style_evaluation = evaluate_style_with_chatgpt(detailed_attributes, api_key=openai_key)
        style_evaluation["color_confidence"] = float(overall_color_confidence)
...
def main():
    ...
    result = process_image(
        ...
        visualize=not args.no_visualize_parsing,
        output_dir="..",
    )
    ...
    # Print style evaluation if available
    if result.get("style_evaluation"):
        print("\n=== STYLE EVALUATION ===")
        se = result["style_evaluation"]
        print(f"Rating: {se.get('rating', 'N/A')}")
        print(f"Score: {se.get('score', 'N/A')}/10")
        print(f"Feedback: {se.get('feedback', 'N/A')}")
        if se.get('suggestions'):
            print("Suggestions:")
